...
| Web 科研辅助分析原型 | ✅ 当前 validation 阶段闭环 | 98% | 首页/上传/健康检查、MPR、10 例人工 QC reviewer、C1–L6 可读标签、真值 PLY WebGL2 3D、简化/物理测量均已完成；QC reviewer 已修复全站 `.card` grid-column 与 QC 网格冲突，病例选择后使用 `hidden + display:none!important` 彻底关闭病例层并进入主审核区，“上一例 / 下一例”保持审核区，悬浮按钮可随时重新打开病例列表；已在本机 Edge 对真实 `liver_0` 完成点击关闭/重新展开实机验证。另有 SDF surface 选择与 evaluation results-review，可读取未来 prediction/entropy MPR。`results-review` 已实机读取真实 v13 validation evaluation：Edge 已显示 prediction MPR overlay 与 predictive-entropy/uncertainty overlay；`research-3d` 已接真实 evaluation prediction，Edge 已加载 liver_8 的 2.0 mm feature-weighted prediction mesh 与 SDF σ=0.4 mm surface，并完成 GT/prediction 双来源切换；`/api/research/cases` 仅暴露 liver_0～liver_8 共 9 例，锁参前继续隔离 `test_private liver_169` |
| 三维重建 | ✅ 当前 validation 阶段闭环 | 98% | 已实现 physical-space Marching Cubes、PLY/JSON、vertex-clustering、SDF surface、WebGL2 与物理测量；新增相邻法向变化驱动的特征保护 vertex-clustering 候选，真实 `liver_0` 在 2.0 mm/同 30,260 顶点下将高特征区域 mean-NN 约 0.679→0.620 mm、HD95 约 1.068→1.000 mm，作为真值网格工程证据；0.4 mm SDF 保持 2→2 连通域，0.8 mm 因 2→3 被保护机制拒绝。已在 v13 validation 真实 prediction surface 上完成工程验证：liver_7/liver_8 的 2.0 mm、feature strength=8 简化分别保留约 298,840/296,483 顶点，顶点缩减约 78.18%/77.89%，简化工程 ASSD/HD95≈0.55845/1.09434 mm 与 0.54806/1.08294 mm；0.4 mm SDF 分别保持 1564→1564、1528→1528 连通域，SDF-vs-original 工程 ASSD/HD95≈0.02919/0.06790 mm 与 0.02929/0.06671 mm。以上均为 engineering surface 指标，不冒充分割临床 HD95/ASSD |
| 论文 | 🟡 进行中 | 78% | 中文技术初稿已补 formal preflight、uncertainty/ROI refinement、calibration、物理表面/特征保护重建；Related Work 已加入 SpineMamba、解剖变异 Transformer、VertebraFormer、2026 Residual-Encoder nnU-Net、2025 骨折 pipeline、金属植入物与低骨密度 fusion/split 困难病例证据；42 条英文 BibTeX / 44 条矩阵已同步。Results 已开始写入真实 engineering/validation 结果，并明确保留 independent test 空位；已覆盖 validation 主结果、输入/loss/sampling/augmentation/difficult-sample 消融、uncertainty/calibration、REFINEMENT=FAIL、prediction 3D/SDF 与 Web prototype。正式 independent test 在锁参前仍不得填写 |
| 中期材料 | 🟡 进行中 | 90% | 已同步 10 例真实数据、97 项测试、10/10 人工 QC、正式 binary task lock、7/2/1 formal-pilot split、`formal_readiness ready=true`，并新增 CPU CT-only 5-epoch formal-pilot checkpoint；validation 阶段的真实主候选、refinement 结论、prediction 3D/SDF 与 Web 实机验收已闭环；仍缺锁参后的正式 independent full-volume test、扩大样本规模后的正式主实验与外部/临床验证 |
