...
| Web 科研辅助分析原型 | ✅ validation + independent test 闭环 | 99% | 首页/上传/健康检查、MPR、10 例人工 QC reviewer、C1–L6 可读标签、真值 PLY WebGL2 3D、简化/物理测量均已完成；QC reviewer 已修复全站 `.card` grid-column 与 QC 网格冲突，病例选择后使用 `hidden + display:none!important` 彻底关闭病例层并进入主审核区，“上一例 / 下一例”保持审核区，悬浮按钮可随时重新打开病例列表；已在本机 Edge 对真实 `liver_0` 完成点击关闭/重新展开实机验证。另有 SDF surface 选择与 evaluation results-review，可读取未来 prediction/entropy MPR。`results-review` 已实机读取真实 v13 validation evaluation：Edge 已显示 prediction MPR overlay 与 predictive-entropy/uncertainty overlay；`research-3d` 已接真实 evaluation prediction，Edge 已加载 liver_8 的 2.0 mm feature-weighted prediction mesh 与 SDF σ=0.4 mm surface，并完成 GT/prediction 双来源切换；锁参前 `/api/research/cases` 始终只暴露 liver_0～liver_8 共 9 例。锁参后正式 independent evaluation 已在 `results-review` 被真实识别，prediction/uncertainty MPR API 均返回 200；`research-3d` 已在 Edge 实机加载 independent liver_169 的 2.0 mm prediction mesh 与 SDF σ=0.4 mm surface |
...
| 论文 | 🟡 进行中 | 92% | 中文技术初稿已补 formal preflight、uncertainty/ROI refinement、calibration、物理表面/特征保护重建；Related Work 已加入 SpineMamba、解剖变异 Transformer、VertebraFormer、2026 Residual-Encoder nnU-Net、2025 骨折 pipeline、金属植入物与低骨密度 fusion/split 困难病例证据；42 条英文 BibTeX / 44 条矩阵已同步。Results 已开始写入真实 engineering/validation 结果，并明确保留 independent test 空位；已覆盖 validation 主结果、输入/loss/sampling/augmentation/difficult-sample 消融、uncertainty/calibration、REFINEMENT=FAIL、prediction 3D/SDF 与 Web prototype。锁参后的唯一一次正式 independent test 已完成，当前正在将真实低性能结果、3D/SDF、Discussion、Failure Cases、Limitations 与 Conclusion 同步进唯一论文文件 |
| 中期材料 | 🟡 进行中 | 96% | 已同步 10 例真实数据、97 项测试、10/10 人工 QC、正式 binary task lock、7/2/1 formal-pilot split、`formal_readiness ready=true`，并新增 CPU CT-only 5-epoch formal-pilot checkpoint；validation 阶段的真实主候选、refinement 结论、prediction 3D/SDF 与 Web 实机验收已闭环；锁参后的正式 independent full-volume test、independent 3D/SDF/Web 已完成；后续主要外部缺口是扩大样本规模后的主实验、合法临床数据与外部验证 |
