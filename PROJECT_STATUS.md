...
- [ ] 从同一 v11 run 的 `last.pt` resume 到总 epoch2；必须验证 encoder delta=`0`、BN running buffer delta=`0`、decoder feature delta=`0`、`linear_pred` delta!=`0`（若真实收到梯度）；
