...
- Active run: `experiments/20260917_185941_ctspine1k_v7_segformer3d_hr_hard_negative_precision`.
- At epoch 11 validation, cache case `ctspine1k-colonog-ctc-1137132466` failed SimpleITK loading; split check confirmed validation=true, test=false.
- Rebuilt that compact-cache case from its registered CTSpine1K source and verified gzip CRC plus image/label shape `267 x 267 x 291`.
- Preserved and resumed the same run from `checkpoint/last.pt`; no restart and no test198 access.
- Resume verified healthy at epoch 11 with live heartbeat/status updates.
- Reworked `v7_monitor_gui.py` into a fixed-size native Tkinter monitor using both `training_status.json` and `training_heartbeat.json`, with 4-second refresh, progress, epoch/step, ETA, phase, loss/Dice/best/lr, update time, and failure-state display.
