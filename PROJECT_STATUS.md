...
| SegFormer3D 骨科适配 | 🟡 进行中 | 95% | adapter、配置、dataset、训练骨架已完成；首个任务已锁定为 `binary_semantic`。v9 证明冻结 BN running stats 可显著缓解 v6 foreground explosion，但不能消除退化；v10 在 encoder 与 BN 全冻结后仍发生 mean Dice≈`1.65e-11` 的 catastrophic background collapse，否定 encoder parameter update 为必要条件。v11 进一步从 epoch2 起冻结 decoder feature（`linear_c1..c4` + `linear_fuse`），仅允许 `linear_pred` 更新：epoch1 mean val Dice=`0.0540700072`，epoch2=`0.0543761681`，没有 collapse。两例 epoch2 detailed Dice≈`0.04421/0.06454`、prediction/GT ratio≈`3.96/3.50`；checkpoint 验证 encoder/BN/decoder-feature delta=`0`，只有 final-head weight+bias 更新。固定 `liver_7` dynamics 中 encoder、decoder fuse 与 head-input activation 完全一致，仅 final logits 改变；两例 diagnostics 也未见 foreground explosion/background collapse。当前证据支持 decoder feature update 是 v10 collapse 的关键机制之一，下一步直接进入 v11 epoch3 验证稳定性 |
...
- [ ] 从 v11 `last.pt` 直接 resume 到总 epoch3；继续验证冻结 tensor、只对 `liver_7/liver_8` 做必要 validation，并据此判定 stable baseline；
