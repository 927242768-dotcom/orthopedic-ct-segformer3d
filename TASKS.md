...
- [ ] v11 采用下一单变量：在 v10 基础上从 epoch2 起进一步冻结 decoder feature parameters（`linear_c1..c4` + `linear_fuse`），仅保留最终 `linear_pred` segmentation head 可训练；继续保持 encoder 与 BN running stats 冻结，其余 lr/loss/sampling/ROI/augmentation 不变，以隔离 decoder representation update 是否为剩余主因
