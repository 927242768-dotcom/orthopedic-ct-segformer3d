...
- [ ] v9 前置机制诊断：比较 v6 epoch1/epoch2 与 v8 epoch1 的关键 encoder/decoder/head-input activation 分布和 checkpoint 参数 delta，定位上游 feature/logit 漂移最显著模块；再据证据只选择一个新变量，禁止同时改 lr/loss/normalization/sampling；独立 test `liver_169` 继续禁止访问
