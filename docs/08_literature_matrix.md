...
- 2025–2026 直接脊柱工作已经进一步推进到 **Mamba + 形状先验、解剖变异感知、结构感知多任务与 domain generalization**；因此本项目不能只用“Transformer + Dice”作为创新点，必须用边界/拓扑、困难样本、不确定性精修和物理三维误差闭环形成可验证差异。
...
3. **spine CT**：VerSe / CTSpine1K / Lessmann / TotalSegmentator / VerFormer → SpineMamba / 2025 anatomical-variation Transformer / 2026 VertebraFormer；
...
本矩阵当前收录 **40 个方法/数据/重建条目（含 2 个国内待数据库二次核验条目）**；`paper/references.bib` 当前有 **38 个英文核心 BibTeX 条目**。本轮新增并核验 2025 SpineMamba、2025 Scientific Reports 椎体分割/解剖变异 Transformer、2026 VertebraFormer，同时保留此前对 CTSpine1K、VerSe、VerFormer、nnFormer、SegFormer3D、TEDS-Net、EDUE 等易错题录的纠正。英文核心条目优先依据官方出版页面、会议 Open Access、出版社页面或 arXiv 原始记录核对。
