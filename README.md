...
当前已经产生多轮 **engineering / validation experiment** 指标，但尚未形成可锁参进入独立 test 的稳定 baseline。2026-08-27 的稳定性实验表明：v6 epoch1 两例 full-volume validation mean Dice≈`0.05407`，epoch2 出现严重 foreground explosion；v8 从初始化开始冻结 BatchNorm3d running statistics 后，epoch1 mean Dice≈`0.0001248` 并转为严重 background collapse，因此 v8 已停止。随后 checkpoint dynamics 显示 v6 epoch1→epoch2 普通参数组变化总体很小，而多处 BN running statistics 与 decoder/head-input activation 明显漂移。当前已实现 v9：epoch1 保持 v6 原始 BN 更新，epoch2 起冻结 epoch1 已建立的 running statistics，作为单变量机制验证；工程回归为 `125 passed`、Ruff clean，formal readiness `ready=true / blocker_count=0`，尚待真实 v9 训练结果。现阶段结论只用于机制诊断，不能作为论文最终性能结果；任何最终 DSC、HD95、ASSD 等仍必须来自锁定参数后的正式独立 test。
