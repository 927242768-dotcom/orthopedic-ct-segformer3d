...
当前已经产生多轮 **engineering / validation experiment** 指标，但尚未形成可锁参进入独立 test 的稳定 baseline。2026-08-27 的稳定性实验表明：v6 epoch1 两例 full-volume validation mean Dice≈`0.05407`，epoch2 出现严重 foreground explosion；v8 从初始化开始冻结 BatchNorm3d running statistics 后，epoch1 mean Dice≈`0.0001248` 并转为严重 background collapse，因此 v8 已停止。随后 checkpoint dynamics 显示 v6 epoch1→epoch2 普通参数组变化总体很小，而多处 BN running statistics 与 decoder/head-input activation 明显漂移。v9 已采用“epoch1 正常建立 BN stats、epoch2 起冻结”的单变量策略；真实 epoch1 已精确复现 v6 epoch1：train loss=`2.5537127597`、mean validation Dice=`0.0540700072`，`liver_7/8` Dice≈`0.04323/0.06491`，9 个 BN 的 `num_batches_tracked=28`，首层 running mean std≈`0.0144923`、running var mean≈`0.0622548`。这说明新策略没有影响 epoch1，当前关键判定点转为 v9 epoch2 是否能在固定 epoch1 BN stats 下避免前景泛滥。现阶段结论只用于机制诊断，不能作为论文最终性能结果；任何最终 DSC、HD95、ASSD 等仍必须来自锁定参数后的正式独立 test。
